...
async function fetchJSON(url) {
  ...
  if (!res.ok) {
    throw new Error(`HTTP ${res.status} — ${url}\nGitHub Pages chưa có file này. Kiểm tra docs/ đã được deploy chưa.`);
  }
  const text = await res.text();
  try {
    return JSON.parse(text);
  } catch(e) {
    throw new Error(`${url} không phải JSON hợp lệ (nhận được HTML).\nGitHub Pages chưa deploy xong hoặc file bị thiếu.`);
  }
}
...
async function loadDataChunks() {
  ...
  function setProgress(pct, msg) {
    ...
    if (!el) return;
    el.innerHTML = `
      <div style="font-size:13px;font-weight:600;margin-bottom:10px">${msg}</div>
      <div style="width:220px;height:8px;background:#dde3ee;border-radius:10px;margin:0 auto;overflow:hidden">
        <div style="width:${pct}%;height:100%;background:var(--primary);border-radius:10px;transition:width .4s"></div>
      </div>
      <div style="font-size:11px;margin-top:6px;color:var(--text3)">${pct}%</div>`;
  }
  ...
  function showError(msg) {
    ...
    if (!el) return;
    el.innerHTML = `
      <div style="color:#c62828;font-weight:700;font-size:14px;margin-bottom:8px">❌ Lỗi tải dữ liệu</div>
      <div style="color:#555;font-size:12px;white-space:pre-line;text-align:left;max-width:320px;margin:0 auto 12px">${msg}</div>
      <button class="btn btn-primary" onclick="location.reload()">🔄 Thử lại</button>`;
  }

  try {
    setProgress(5, 'Đang kết nối...');
    ...
    if (total < 1) { showError('manifest.json không hợp lệ: chunks = ' + total); return; }

    allData = [];
    for (let i = 0; i < total; i++) {
      const pct = Math.round(10 + (i / total) * 85);
      setProgress(pct, `Đang tải dữ liệu… ${i + 1}/${total} phần`);
      try {
        const chunk = await fetchJSON(`data_${i}.json`);
        if (!Array.isArray(chunk)) throw new Error('Không phải array');
        allData.push(...chunk);
      } catch(e) {
        showError(`Không tải được data_${i}.json\n${e.message}`);
        return;
      }
    }

    setProgress(100, `✅ Đã tải ${allData.length.toLocaleString('vi')} mục thuốc`);
    ...
    console.error('loadDataChunks:', err);
